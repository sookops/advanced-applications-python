...
class Ball(pygame.sprite.Sprite):
     def __init__(self, color, pos, size, speed=(0,0)):
     ...
     def update(self):
          ...
          if self.rect.top > HEIGHT:
               num_live_ball -= 1              # 생명 감소
               self.rect.center = (250,300)  # 공의 초기 위치
               self.vector = [random.randint(3,6)*random.choice([-1,1]), -random.randint(3,6)] 
...
class Paddle(pygame.sprite.Sprite):
     def __init__(self, color, pos, size):
          super().__init__()            # 부모 클래스(Sprite) 초기화
          self.image = pygame.Surface(size, pygame.SRCALPHA)
          pygame.draw.rect(self.image, color, (0,0)+size, 0)
          self.rect = self.image.get_rect()    # Rect 객체
...
class Block(pygame.sprite.Sprite):
     def __init__(self, color, pos, size):
          super().__init__()            # 부모 클래스(Sprite) 초기화
          self.image = pygame.Surface(size, pygame.SRCALPHA)
          pygame.draw.rect(self.image, color, (0,0)+size, 0)     # 공 이미지
          self.rect = self.image.get_rect()                      # Rect 객체
          self.rect.topleft = pos
...
# 게임 초기화
def init_game():
     global game_over, game_speed, block_group, num_live_ball
     ball.rect.center = (250,300)
     ball.vector = [random.randint(3,6)*random.choice([-1,1]), -random.randint(3,6)]
     paddle.rect.centerx = 250
     block_group = pygame.sprite.Group()         # 벽돌 그룹
     ypos = 0
     for color in COLOR:      # 벽돌 배치
          for xpos in range(7):
               block_group.add(Block(color, (xpos*60+40, ypos*30+20), (55, 25)))
          ypos += 1
     game_over = False        # 게임 진행 변수
     game_speed = 60          # 게임 속도
     num_live_ball = 3        # 남은 생명의 수
...
def draw_live_ball():
     xpos = WIDTH - 15
     for i in range(1, num_live_ball):
          pygame.draw.ellipse(win, RED, (xpos, 580, 10, 10))
          xpos -= 15
     
# Main Program
ball = Ball(RED, (250,300), (10,10), (5, -6))     # 공 객체 생성
paddle = Paddle(GREEN, (200,580), (200,10))       # 패들 객체 생성
sprite_group = pygame.sprite.Group()
sprite_group.add(paddle, ball)
init_game()      # 게임 초기화
...
while True:
     ...
     if not game_over:             # 게임 진행
          ...
          if paddle in pygame.sprite.spritecollide(ball, sprite_group, False):
               ball.speed[1] *= -1
          if len(block_group) == 0 or num_live_ball == 0: # 남은 벽돌이 없으면 게임 종료
               game_over = True          
